...
//!
//! This is a **library** with a two-line `src/main.rs` on top of it, because the
//! clap layer has two callers: the `latiq` executable (the container image,
//! `./dev.sh`) and the `latiq` console script the PyPI wheel installs, which
//! reaches [`run_from_args`] through PyO3 (`sdk/python/src/lib.rs`). Everything
//! below stays private; [`run_from_args`] / [`run_from_env`] are the whole API.
//!
//! Two builds ship from here. The default (`server`) is the full binary — the
//! `latiq` wheel, the image, `./dev.sh`. `--no-default-features` is the lean
//! operator CLI published as the `latiq-admin` wheel: no control plane, no pond
//! node, and therefore no DuckDB. In that build `serve` and `node add` still
//! *parse* — they fail with an error naming `pip install latiq` rather than
//! clap's "unrecognized subcommand", which would tell an operator the command
//! does not exist when what is true is that this build cannot run it.
...
/// The error a **client-only** build returns for a server role. It is not
/// "unknown command": the command is real, this build cannot run it, and the fix
/// is a different install — so the message says what this build is, what it
/// cannot do, and the two commands that can (invariant 13: an error names the
/// next action).
///
/// Deliberately NOT an [`ErrorEnvelope`]: that shape is the *surface* contract,
/// whose `see` must resolve to a `latiq://` MCP resource about the kind. Nothing
/// was asked of a server here — there is no server — so there is no kind to
/// carry and no resource to point at. The actionable part is the install.
///
/// Compiled into **both** builds even though only the client-only one can reach
/// it, so `error_contract_server_role_unavailable_names_the_install_that_can`
/// runs under a plain `cargo test --workspace` instead of only under the lean
/// build nobody remembers to test.
#[cfg_attr(feature = "server", allow(dead_code))]
fn server_role_unavailable(role: &str) -> anyhow::Error {
    anyhow!(
        "`latiq {role}` needs the full build; this is latiq-admin, the lean operator CLI \
         (client commands only — no control plane, no pond node, no query engine).\n\
         \n\
         To run a server:\n\
         \x20 pip install latiq       # then: latiq {role} …   (SDK + engine + server roles)\n\
         \x20 docker compose up -d    # the published images: \
         https://github.com/neonexia/latiq/blob/main/deploy/README.md\n\
         \n\
         To drive a cluster someone else runs, this build is the right one — set \
         LATIQ_SERVER and use `latiq pond list` / `latiq stats`."
    )
}

/// `serve`'s arguments in a client-only build: swallowed, not parsed. The point
/// is that `latiq serve --bind 0.0.0.0 --port 51400` reaches
/// [`server_role_unavailable`] instead of dying on clap's "unexpected argument
/// '--bind'", which blames the flag for a problem that is the install.
...
/// As [`ServeArgs`] above, for `node add`.
#[cfg(not(feature = "server"))]
#[derive(Args)]
struct NodeAddArgs {
    #[arg(trailing_var_arg = true, allow_hyphen_values = true, hide = true)]
...
/// Run the CLI over this process's real arguments. What `src/main.rs` calls, and
/// what the wheel's `latiq` console script reaches through PyO3.
pub fn run_from_env() -> Result<()> {
    run_from_args(std::env::args_os())
}

/// Run the CLI over an explicit argument vector (`argv[0]` included, as clap
/// expects). The runtime is built HERE rather than by a `#[tokio::main]` on
/// `main`, because the wheel's entry point is a plain synchronous function
/// called from CPython and must own the runtime the same way the binary does —
/// one construction site, so `latiq serve` behaves identically from either.
...
async fn dispatch(cli: Cli) -> Result<()> {
    ...
    match cli.command {
        ...
        #[cfg(not(feature = "server"))]
        Command::Serve(_) => Err(server_role_unavailable("serve")),
        #[cfg(feature = "server")]
        Command::Node(NodeCmd::Add(a)) => run_node_add(*a).await,
        #[cfg(not(feature = "server"))]
        Command::Node(NodeCmd::Add(_)) => Err(server_role_unavailable("node add")),
...
mod tests {
    ...
    /// The client-only (`latiq-admin`) build must still PARSE `serve` and
    /// `node add`, flags and all. If they were `#[cfg]`d away, clap would answer
    /// `unrecognized subcommand 'serve'` — telling an operator the command does
    /// not exist, when the truth is that this install cannot run it and a
    /// different one can. Asserted through `Cli::try_parse_from`, i.e. the real
    /// parser, so a stub that stopped swallowing `--bind` would fail here.
    #[cfg(not(feature = "server"))]
    #[test]
    fn error_contract_client_only_build_parses_the_server_roles() {
        for argv in [
            vec!["latiq", "serve", "--bind", "0.0.0.0", "--port", "51400"],
            vec!["latiq", "node", "add", "--node-id", "n1", "--port", "51401"],
        ] {
            Cli::try_parse_from(&argv).unwrap_or_else(|e| {
                panic!("{argv:?} must parse in the client-only build, got:\n{e}")
            });
        }
    }

    /// Invariant 13 for the packaging boundary: the refusal names the next
    /// action. Pinned on the strings an operator acts on — the command that
    /// installs the build which CAN serve, and the one that runs the published
    /// images — because a message that only said "unavailable" would leave a
    /// human to guess which of the two channels to reach for.
    ///
    /// Runs in BOTH builds (the function is compiled either way) so the wording
    /// cannot rot in the default build where nobody reads it.
    #[test]
    fn error_contract_server_role_unavailable_names_the_install_that_can() {
        let msg = server_role_unavailable("serve").to_string();
        for needle in [
            "latiq serve",
            "pip install latiq",
            "docker compose up",
            ...
            assert!(
                msg.contains(needle),
                "the client-only `serve` refusal must name `{needle}`, so the reader \
                 knows what to install instead; got:\n{msg}"
            );
        }
    }
