...
- [ ] **The advertised MCP URL is always `http://`.** `advertised_mcp_url` takes scheme and host from `internal_endpoint`, which is always plaintext, so a deployment whose gateway terminates TLS advertises `http://<host>:<mcp-port>/mcp` in both the RFC 9728 document and the challenge. A conforming client that compares the resource identifier against the `https://` URL it dialled rejects the document. Needs an explicit advertised-MCP-URL flag on `NodeAddArgs` + `PondNodeConfig`; the `serve_mcp` parameter is already in place for it. Not exercised by the nightly, which is plaintext end to end — so this will only ever be found in a real deployment.
- [ ] **Discovery breaks under a path-rewriting gateway.** Metadata is served at the node's root `/.well-known/oauth-protected-resource`; a gateway that rewrites paths rather than only hosts would not route it. Noted in a code comment, unsolved.
- [ ] **`ops.rs::audit` diverges in meaning from the Admin emitter.** Admin now records every attempt with an `outcome` field, including rejections; `ops.rs` audits only after success, so a failed or rejected agent action leaves no record at all. Same field names, different semantics, one searchable stream. Align `ops.rs`: emit on failure too, with `outcome`, and add rejection records on `data_service.rs`'s auth failure paths to match Admin.
