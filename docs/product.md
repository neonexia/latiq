...
**Bring in curated data.** A **dataset** is a curated file or set of files (parquet/CSV) that `load_dataset` copies into the pond under its own schema; operators pre-register those. A **catalog** is an external database or lakehouse — Iceberg or DuckLake today — that the agent **attaches** to its pond under a name it chooses. From then on it is ordinary SQL: the agent extracts the subset it needs with a normal `write_query`, and because the attachment persists, it can attach TWO and **join across them in one statement**. Then it detaches. The credential is the agent's own bearer by default (the catalog's API consumes it directly), so in the common case there is no credential to hand out and none to store; when one is needed it is supplied at attach, never logged, and returned by no surface.

**Read the common formats with nothing installed.** A pond is batteries-included on the DuckDB extension set, and that is a value proposition, not plumbing: **CSV, Parquet and JSON** read straight from SQL, over **http(s):// and s3://** as well as local paths, with time zones and Unicode collation on — in every pond, with no operator step. **Geospatial** (GeoJSON/Shapefile and the `ST_*` functions), **full-text search** and **IP/CIDR types** are requested at allocation (`allocate_pond { extensions: [...] }`, fixed for the pond's life), and **Iceberg** is reached through `attach_catalog`. The split is deliberate: installing costs the node disk, *loading* costs every pond memory, so the always-loaded set stays at what the product is built on and the rest is installed on every node and loaded only where it was asked for. The whole list, with what each one unlocks and how to get it, is served to agents at `latiq://dialect` — generated from the shipped set, so it cannot advertise something a node does not have.
