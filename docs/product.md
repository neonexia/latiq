...
**Read the common formats with nothing installed.** A pond is batteries-included on the DuckDB extension set, and that is a value proposition, not plumbing: **CSV, Parquet and JSON** read straight from SQL, over **http(s):// and s3://** as well as local paths, with time zones and Unicode collation on — in every pond, with no operator step. **Geospatial** (GeoJSON/Shapefile and the `ST_*` functions), **full-text search** and **IP/CIDR types** are requested at allocation (`allocate_pond { extensions: [...] }`, fixed for the pond's life), and **Iceberg** is reached through `pull_catalog`. The split is deliberate: installing costs the node disk, *loading* costs every pond memory, so the always-loaded set stays at what the product is built on and the rest is installed on every node and loaded only where it was asked for. The whole list, with what each one unlocks and how to get it, is served to agents at `latiq://dialect` — generated from the shipped set, so it cannot advertise something a node does not have.
