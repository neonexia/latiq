...
- **A one-shot test runner must not share a profile with the stack it tests.** `auth.env` sets `COMPOSE_PROFILES=auth`, so `auth-tests-sdk`/`auth-tests-agent` sitting in `auth` made `up -d` start them *immediately* — before Keycloak was listening. Nine agent tests "failed" with `fetch failed` in a container nobody meant to run, and the real `run --rm` step re-ran everything afterwards over the same mounted `/repo`. They live in `auth-tests` now; `docker compose run` enables a service's own profiles, so nothing else changed.
- **Install a locally built wheel by its PATH, not by name.** `pip install --no-index --find-links dist latiq` cannot resolve the wheel's *own* third-party deps (`No matching distribution found for pyarrow>=14`) because `--no-index` blocks the whole resolution, not just latiq. `pip install /repo/dist/latiq-*.whl -r e2e/sdk/requirements.txt` is the shape: latiq can only come from that file, deps come from PyPI. `auth-tests-sdk` then asserts PEP 610's `direct_url.json` points into `/repo/dist` — a version compare cannot tell the local build from the published one, they share a version.
