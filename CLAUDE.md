...
13. **Errors are actionables — an agent must be able to decide what to do next without a human.** Four fields carry that without prose: **`audience`** (`agent`/`operator` — who can fix it; two-valued because a variant no envelope can carry is a dead variant), **`retryable`** (`never`/`as_is`/`after_change`/`after_provisioning` — the field that stops an agent looping on a statement that can never succeed; `never` is scoped to *this call*, which is why `read_only_violation` is `never` and the move is `write_query`, while `after_provisioning` says the call was RIGHT and this deployment is missing a capability, so the agent surfaces it to whoever orchestrates it — a human or a higher-privilege agent — and stops rather than retrying), **`facts`** (the numbers and names as values) and **`trace_id`**/**`traceparent`**. **Every one of them is documented where an agent reads** — `latiq://guidance`, the troubleshooting index and the server instructions — because a field no tool description, resource or prompt mentions is a control channel with no receiver: an LLM reads `message` and `suggest`, which is the prose it already had. `latiq-mcp`'s `error_contract_guidance_documents_every_envelope_control_field` enumerates `Retryable::ALL`/`Audience::ALL` against that text, the same shape as the `see`-resolves guard, so the documentation cannot fall behind a new variant. Both defaults are derived per kind in `latiq-common` and must agree with that kind's `suggest`. **`message` is RENDERED FROM `facts`** (`ErrorEnvelope::rendered`, `{name}` placeholders) wherever it quotes a value — prose and numbers cannot drift if the prose is generated from the numbers, which is the mechanism behind "a number we report must be one we measured". A message we did NOT compose (an engine's own error string) carries **no facts** rather than invented ones — with one narrow exception, `unsupported_feature`'s `feature`, which is **lifted verbatim out of the engine's own sentence** so a client can branch on the value; a fact we could not lift is not published at all. There is deliberately **no `next` field**: `suggest` is it. The shape is pinned by the vendored JSON Schema in `crates/latiq-common/spec/` — rmcp does not validate tool responses, so that guarantee is ours. Beyond that: the `kind` says what went wrong, the `suggest` names the **next call** (`describe_pond`, `read_query "SHOW TABLES"`, a corrected statement), and the `see` resolves to a resource **about that kind** (pinned by `latiq-mcp`'s `error_contract_every_error_kind_sees_a_resource_that_exists`). Three rules follow, each from a shipped bug: **classify by what went wrong, never by which call failed** — the engine adapter maps DuckDB's error *class* (`Parser Error`/`Catalog Error`/`Binder Error`/`Conversion Error`/`Constraint Error`/`IO Error`/`Not implemented Error`/`Invalid Input Error`/`Out of Range Error`/`TransactionContext Error`) onto `EngineError`, because classifying by call site made the kind an accident of the binder's phasing — and because the classes are a FINITE set, the ones left in the catch-all are enumerated with their reasons (`errclass.rs::class_of`) instead of defaulted: `Not implemented Error` sat there for months and told agents to retry `CREATE TABLE t(id INTEGER PRIMARY KEY)` for ever (Nexus finding 8); **`internal` + "retry, then report to your operator" is only for OUR failures** — never for anything the caller can fix, since it asks an agent to repeat a statement that can never succeed and then to wake a human; and **a kind nobody can reach is not a feature** — a test that fabricates state to assert a kind passes while that kind is dead in production, so trigger the real condition (stop the node). The mechanical half of the `internal` rule is enforced at the construction funnel, not by a list of sites: **an `internal` envelope may not carry a bespoke `suggest`, and its `see` must be internal's own page** (`latiq-common`'s `assert_opaque_kinds_keep_canonical_guidance`, a `debug_assert` so it costs a release build nothing). `internal` is not a severity, it is a statement about knowledge — the bucket for a failure we could not classify — so a site with a specific next step has, by that fact, classified the failure and owes it a kind whose `audience`/`retryable` match its own advice. Enumerating construction sites is what failed before: every guard iterated `ErrorKind::ALL` and read the KIND TABLE, while all three shipped defects lived in a constructed envelope that overrode it.
...
- `cargo clippy --workspace --all-targets -- -D warnings` and `cargo fmt --all` — keep green (run manually before pushing). **Lint against the same toolchain CI uses**: CI is `dtolnay/rust-toolchain@stable`, i.e. *today's* stable, which is routinely ahead of a local toolchain that hasn't been `rustup update`d. New clippy releases add lints, so **local green does not imply CI green** — this has already cost a red nightly (`result_large_err` firing on tonic-generated code under a stable four releases newer than the local one). Before pushing: `rustup update stable && cargo +stable clippy --workspace --all-targets -- -D warnings`. CI is **nightly only** (`.github/workflows/nightly.yml`), not per-PR, to bound GitHub usage (#28): fmt+clippy+test, the iceberg/MinIO catalog e2e, a dockerized cluster scale-out, the **`e2e/` end-to-end suite** (SDK + MCP agent harness + perf, against a gatewayed multi-node cluster), and a **test-gated + change-gated versioned publish** (two PyPI wheels — `latiq` (SDK + full CLI) and `latiq-admin` (lean operator CLI) — + GHCR images; no `curl | sh` installer, no release-asset binaries — `deploy/CLAUDE.md`, `docs/releasing.md`). Those checks live in **one reusable workflow** (`.github/workflows/verify.yml`) which both `nightly.yml` and `release.yml` (a `v*` tag) call, so a tagged release runs exactly what the nightly runs; **no publishing job may skip it** (#55). The **images** are factored out the same way (`.github/workflows/images.yml`), and are **multi-arch (`linux/amd64` + `linux/arm64`)**: `latiq` builds once per arch on a *native* runner (it compiles DuckDB — QEMU would take hours), the digests merge into one manifest list, and the job then re-reads the published manifest from ghcr.io and fails unless both platforms are in it (#66). The **wheels** are factored out the same way (`.github/workflows/wheels.yml`) and are **multi-platform** (manylinux `x86_64` + `aarch64`, macOS `arm64`) for *both* PyPI projects, each built on a native runner; the built `dist/` is asserted against the required platform list, the published PyPI file list is asserted after an upload, and both wheels are pip-installed and driven on arm64 runners (#108). **No sdist**, deliberately — see `docs/releasing.md`. **`deploy/` is the single home for deployment artifacts** — `deploy/docker-compose.yml` (the clone-free user deployment), `deploy/cluster/` (the multi-node CI/dev stack: pond nodes behind an **nginx gateway** = the single MCP + Data/Stream front door), `deploy/iceberg-minio/` (the catalog fixture) and both Dockerfiles. Start at `deploy/README.md`.

### Disk hygiene — `target/` is enormous here, and running out stops everything

Every test binary statically links a bundled DuckDB (~130–160 MB) and there are 15
of them per profile (`crates/latiq/tests/CLAUDE.md` rule 5). Measured on this repo:
a **single** `target/` reached **56 GB** after repeated multi-profile builds, and a
merged-and-forgotten agent worktree sat at **10–16 GB** each. Two disk exhaustions
have stopped work mid-task.

The failure mode is the reason this is a rule and not a preference: when the volume
fills, the agent tool harness **cannot write its own output file**, so `df`, `rm`,
and every other command fail before running. You cannot clear it from inside a
session — it has to be done from a plain terminal.

So:

- **Delete a worktree's `target/` as part of merging its PR**, not "later". A merged
  worktree has no reason to keep a build tree.
- **Keep at most ~2 agent worktrees alive at once.** Each is a full build tree.
- Reclaim, cheapest first — none of these lose source, only build output:
  ```bash
  rm -rf .claude/worktrees/*/target     # merged/idle worktrees
  rm -rf target                         # main tree (costs one slow DuckDB rebuild)
  ```
- `du -sh target .claude/worktrees/*/target` before starting a long multi-agent run.
