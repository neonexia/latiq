...
13. **Errors are actionables — an agent must be able to decide what to do next without a human.** The `kind` says what went wrong, the `suggest` names the **next call** (`describe_pond`, `read_query "SHOW TABLES"`, a corrected statement), and the `see` resolves to a resource **about that kind** (pinned by `latiq-mcp`'s `error_contract_every_error_kind_sees_a_resource_that_exists`). Three rules follow, each from a shipped bug: **classify by what went wrong, never by which call failed** — the engine adapter maps DuckDB's error *class* (`Parser Error`/`Catalog Error`/`Binder Error`/`Conversion Error`/`Constraint Error`/`IO Error`) onto `EngineError`, because classifying by call site made the kind an accident of the binder's phasing; **`internal` + "retry, then report to your operator" is only for OUR failures** — never for anything the caller can fix, since it asks an agent to repeat a statement that can never succeed and then to wake a human; and **a kind nobody can reach is not a feature** — a test that fabricates state to assert a kind passes while that kind is dead in production, so trigger the real condition (stop the node).
