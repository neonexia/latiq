...
# Ports are overridable (LATIQ_*_ADDR) so you can run alongside other services.
set -euo pipefail
cd "$(dirname "$0")"
...
CONTROL_ADDR=${LATIQ_CONTROL_ADDR:-127.0.0.1:9090}
ADMIN_ADDR=${LATIQ_ADMIN_ADDR:-127.0.0.1:9091}
MCP_ADDR=${LATIQ_MCP_ADDR:-127.0.0.1:8080}
DATA_ADDR=${LATIQ_DATA_ADDR:-127.0.0.1:8081}
...
check_port() {
  ...
  if lsof -nP -iTCP:"$port" -sTCP:LISTEN >/dev/null 2>&1; then
    ...
    echo "Free it, or pick another port, e.g. LATIQ_CONTROL_ADDR=127.0.0.1:19090 ./dev.sh" >&2
    exit 1
  fi
}
